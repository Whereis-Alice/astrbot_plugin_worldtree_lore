...
- 参考：[六种条目模板](#六种条目模板) · [关键词写法](#关键词写法) · [适用范围](#适用范围) · [Cron 规则](#cron-规则) · [内容占位符](#内容占位符) · [插件配置](#插件配置)
- [常见问题](#常见问题)
- [数据与安全](#数据与安全)
- [开发与自查](#开发与自查)
- [致谢与许可](#致谢与许可)
...
模板只做两件事：新建条目时填一套预设值，以及给条目一个可筛选、可排序的分类徽章。它不是六条独立的注入通道——六种模板共用同一套字段和同一套判定逻辑，所以任何条目都能再手动改成别的行为。
